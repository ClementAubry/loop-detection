...
//      plotTime->addBox(idBoxForMap++, *it,outside);
      plotTime->addBox(idBoxForMap++, *it,Qt::blue);
      volumeNO += (*it).volume();
    }
  for (it = YES.begin(); it != YES.end() ; it++){
//      plotTime->addBox(idBoxForMap++, *it,inside);
      plotTime->addBox(idBoxForMap++, *it,Qt::red);
      volumeYES += (*it).volume();
    }
  ...
  plotTime->update();
  //display infos
  ui->infoBox->insertPlainText("\nLoop Detection resolved in " + QString::number(difftime(tEnd,tStart),'f',2) + " seconds");
  ui->infoBox->insertPlainText("\n----------------------------------------------------------");
...
{
  computeKalmanTPlaneDistSeuil();
  //    computeKalmanTPlaneDistSeuilSeeDelay();
  //  computeKalmanTPlaneDistNormalized();
}
void MainWindow::computeKalmanTPlaneDistSeuil()
{
  //test affichage données kalman : OK
  //    for (uint k=0;k<estimX.size();k++){
  //        plotPosition->addBoldPoint(estimX[k], estimY[k], QColor("blue"));
  //    }
  //"profiling" purpose
  QTime temps;
  temps.start();
  ...
  for (uint ii=0;ii<estimX.size();ii++){
      for (uint jj=0;jj<ii;jj++){
          QColor cc=GetColour(0,0,seuil);
          plotTime->addKalmanPoint(ii,jj,cc);
        }
    }
  for (uint ii=0;ii<estimX.size();ii++){
      for (uint jj=ii;jj<estimX.size();jj++){
  ...
  plotTime->update();
  ui->infoBox->insertPlainText("\nLD via Kalman Filtering resolved and drawn in "+QString::number(temps.elapsed())+" ms for "+QString::number(plotTime->kalmanPointList.size(),'d',0)+" t-pairs");
  QScrollBar *sb = ui->infoBox->verticalScrollBar();
  sb->setValue(sb->maximum());
}
void MainWindow::computeKalmanTPlaneDistSeuilSeeDelay()
{
  ...
  plotTime->update();
  ui->infoBox->insertPlainText("\nLD via Kalman Filtering resolved and drawn in "+QString::number(temps.elapsed())+" seconds for "+QString::number(plotTime->kalmanPointList.size(),'d',0)+" t-pairs");
  QScrollBar *sb = ui->infoBox->verticalScrollBar();
  sb->setValue(sb->maximum());
}
//compute and draw LDKalman by euclidean distance
void MainWindow::computeKalmanTPlaneDistNormalized()
{
  QTime temps;
  temps.start();
  double seuil = ui->seuilDistKalmanStepSpinBox->value();
  double maxDist = 0;
  vector<vector<double> > tplanDistNrmlz;
  tplanDistNrmlz.reserve(estimX.size());
  for (uint ii=0;ii<estimX.size();ii++){
      vector<double> temp;
      temp.reserve(estimY.size());
      for (uint jj=0;jj<ii;jj++){
          temp.push_back(1);
        }
      for (uint jj=ii;jj<estimX.size();jj++){
      ...
      tplanDistNrmlz.push_back(temp);
    }
  ui->infoBox->insertPlainText("\nLD via Kalman Filtering computed in "+QString::number(temps.elapsed())+" seconds for "+QString::number(plotTime->kalmanPointList.size(),'d',0)+" t-pairs");
  for (uint ii=0;ii<tplanDistNrmlz.size();ii++){
      for (uint jj=0;jj<ii;jj++){
          QColor cc=GetColour(0,0,maxDist);
          plotTime->addKalmanPoint(ii,jj,cc);
        }
      for (uint jj=ii;jj<tplanDistNrmlz.at(ii).size();jj++){
          QColor cc=GetColour(maxDist-tplanDistNrmlz[ii][jj],0,maxDist);
          plotTime->addKalmanPoint(ii,jj,cc);
        }
    }
  plotTime->update();
